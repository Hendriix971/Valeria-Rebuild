...
const ARMORS={armure_legere:{id:'armure_legere',name:'Armure légère',emoji:'🦺',level:1,stats:{def:30,esq:3},materials:{peau_sanglier:2,fourrure_loup:1},normalPrice:80,reducedPrice:40,sellPrice:20,desc:'+30 DEF, +3% ESQ'},armure_lourde:{id:'armure_lourde',name:'Armure lourde',emoji:'🛡️',level:1,stats:{def:50,pvMax:150},materials:{morceau_ferraille:3,défense_sanglier:1},normalPrice:100,reducedPrice:50,sellPrice:25,desc:'+50 DEF, +150 PV max'},robe_magique:{id:'robe_magique',name:'Robe magique',emoji:'👘',level:1,stats:{def:30,peMax:150},materials:{soie_araignée:2,noyau_slime:1},normalPrice:100,reducedPrice:50,sellPrice:25,desc:'+30 DEF, +150 PE max'},tunique:{id:'tunique',name:'Tunique',emoji:'👕',level:1,stats:{pvMax:125,peMax:125},materials:{fourrure_loup:2,peau_sanglier:1},normalPrice:90,reducedPrice:45,sellPrice:22,desc:'+125 PV max, +125 PE max'},armure_legere_renforcee:{id:'armure_legere_renforcee',name:'Armure légère renforcée',emoji:'🦺',level:10,stats:{def:60,esq:4},materials:{cuir_epais:2,fourrure_epaisse:2,patte_chitineuse:1},normalPrice:300,reducedPrice:150,sellPrice:75,desc:'+60 DEF, +4% ESQ'},armure_lourde_cuirassee:{id:'armure_lourde_cuirassee',name:'Armure lourde cuirassée',emoji:'🛡️',level:10,stats:{def:100,pvMax:300},materials:{plaque_osseuse:2,défense_massive:1,cuir_renforce:2},normalPrice:350,reducedPrice:175,sellPrice:88,desc:'+100 DEF, +300 PV max'},robe_obscure:{id:'robe_obscure',name:'Robe obscure',emoji:'👘',level:10,stats:{def:60,peMax:300},materials:{soie_noire:2,noyau_corrompu:1,écaille_nocturne:1},normalPrice:350,reducedPrice:175,sellPrice:88,desc:'+60 DEF, +300 PE max'},tunique_renforcee:{id:'tunique_renforcee',name:'Tunique renforcée',emoji:'👕',level:10,stats:{pvMax:250,peMax:250},materials:{fourrure_epaisse:2,cuir_epais:1,sang_froid:1},normalPrice:320,reducedPrice:160,sellPrice:80,desc:'+250 PV max, +250 PE max'},armure_harpie:{id:'armure_harpie',name:'Armure de harpie',emoji:'🦺',level:20,stats:{def:120,esq:5},materials:{plume_tranchante:4,serre_harpie:2,bois_spectral:1},normalPrice:1200,reducedPrice:600,sellPrice:300,desc:'+120 DEF, +5% ESQ'},armure_gargouille:{id:'armure_gargouille',name:'Armure de gargouille',emoji:'🛡️',level:20,stats:{def:200,pvMax:600},materials:{fragment_gargouille:4,mousse_petrifiee:2,coeur_epines:1},normalPrice:1500,reducedPrice:750,sellPrice:375,desc:'+200 DEF, +600 PV max'},robe_follet:{id:'robe_follet',name:'Robe du feu follet',emoji:'👘',level:20,stats:{def:120,peMax:600},materials:{flamme_spectrale:3,essence_lumineuse:2,linceul_ancien:1},normalPrice:1500,reducedPrice:750,sellPrice:375,desc:'+120 DEF, +600 PE max'},tunique_mandragore:{id:'tunique_mandragore',name:'Tunique de mandragore',emoji:'👕',level:20,stats:{pvMax:500,peMax:500},materials:{racine_hurlante:3,seve_amere:2,spores_geantes:3},normalPrice:1300,reducedPrice:650,sellPrice:325,desc:'+500 PV max, +500 PE max'},armure_incandescente:{id:'armure_incandescente',name:'Armure incandescente',emoji:'🦺',level:30,stats:{def:240,esq:6},materials:{aile_incandescente:4,griffe_incandescente:2,ecaille_lave:3},normalPrice:3600,reducedPrice:1800,sellPrice:900,desc:'+240 DEF, +6% ESQ'},armure_basalte:{id:'armure_basalte',name:'Armure de basalte',emoji:'🛡️',level:30,stats:{def:400,pvMax:1200},materials:{fragment_basalte:4,coeur_basalte:2,ecaille_obsidienne:3},normalPrice:4500,reducedPrice:2250,sellPrice:1125,desc:'+400 DEF, +1200 PV max'},robe_braise:{id:'robe_braise',name:'Robe de braise',emoji:'👘',level:30,stats:{def:240,peMax:1200},materials:{coeur_braise:3,braise_eternelle:1,linceul_brule:2},normalPrice:4500,reducedPrice:2250,sellPrice:1125,desc:'+240 DEF, +1200 PE max'},tunique_lave:{id:'tunique_lave',name:'Tunique de lave',emoji:'👕',level:30,stats:{pvMax:1000,peMax:1000},materials:{ecaille_lave:3,cendre_vive:4,roche_fondue:3},normalPrice:4000,reducedPrice:2000,sellPrice:1000,desc:'+1000 PV max, +1000 PE max'}}
const MONSTERS={slime:{id:'slime',name:'Slime',emoji:'🟢',baseStats:{for:1,rap:1,con:4,mana:1},drops:[{id:'gelée_slime',min:1,max:2,chance:.8},{id:'noyau_slime',min:1,max:1,chance:.3}],xpReward:15},loup:{id:'loup',name:'Loup',emoji:'🐺',baseStats:{for:3,rap:4,con:2,mana:1},drops:[{id:'croc_loup',min:1,max:2,chance:.7},{id:'fourrure_loup',min:1,max:1,chance:.5},{id:'griffe_loup',min:1,max:1,chance:.4}],xpReward:18},sanglier:{id:'sanglier',name:'Sanglier',emoji:'🐗',baseStats:{for:4,rap:2,con:5,mana:1},drops:[{id:'peau_sanglier',min:1,max:2,chance:.7},{id:'défense_sanglier',min:1,max:1,chance:.4}],xpReward:22},araignee:{id:'araignee',name:'Araignée',emoji:'🕷️',baseStats:{for:2,rap:3,con:3,mana:2},drops:[{id:'soie_araignée',min:1,max:2,chance:.7},{id:'venin_araignée',min:1,max:1,chance:.4}],xpReward:20},serpent:{id:'serpent',name:'Serpent',emoji:'🐍',baseStats:{for:3,rap:4,con:2,mana:2},drops:[{id:'écaille_serpent',min:1,max:2,chance:.7},{id:'croc_serpent',min:1,max:1,chance:.4}],xpReward:24},treant:{id:'treant',name:'Tréant',emoji:'🌳',baseStats:{for:5,rap:1,con:6,mana:2},drops:[{id:'bois_solide',min:2,max:3,chance:.8},{id:'écorce_ancienne',min:1,max:1,chance:.3}],xpReward:30},gobelin_faible:{id:'gobelin_faible',name:'Gobelin',emoji:'👺',baseStats:{for:2,rap:3,con:2,mana:1},drops:[{id:'morceau_ferraille',min:1,max:2,chance:.6},{id:'os_monstre',min:1,max:1,chance:.5}],xpReward:16},loup_alpha:{id:'loup_alpha',name:'Loup alpha',emoji:'🐺',baseStats:{for:4,rap:5,con:3,mana:1},drops:[{id:'croc_loup',min:2,max:3,chance:.8},{id:'fourrure_loup',min:1,max:2,chance:.6}],xpReward:28},gobelin_eclaireur:{id:'gobelin_eclaireur',name:'Gobelin éclaireur',emoji:'👹',baseStats:{for:4,rap:5,con:3,mana:2},drops:[{id:'morceau_ferraille',min:1,max:3,chance:.7},{id:'os_monstre',min:1,max:2,chance:.6}],xpReward:26},gobelin_guerrier:{id:'gobelin_guerrier',name:'Gobelin guerrier',emoji:'👺',baseStats:{for:5,rap:4,con:4,mana:2},drops:[{id:'lame_rouillee',min:1,max:2,chance:.65},{id:'cuir_gobelin',min:1,max:2,chance:.55},{id:'os_renforce',min:1,max:1,chance:.35}],xpReward:38},loup_sanguinaire:{id:'loup_sanguinaire',name:'Loup sanguinaire',emoji:'🐺',baseStats:{for:5,rap:6,con:3,mana:1},drops:[{id:'croc_sanguinaire',min:1,max:2,chance:.7},{id:'fourrure_epaisse',min:1,max:2,chance:.6},{id:'griffe_aceree',min:1,max:1,chance:.4}],xpReward:42},slime_corrompu:{id:'slime_corrompu',name:'Slime corrompu',emoji:'🟣',baseStats:{for:3,rap:2,con:6,mana:4},drops:[{id:'gelée_corrompue',min:1,max:2,chance:.75},{id:'noyau_corrompu',min:1,max:1,chance:.4},{id:'résidu_instable',min:1,max:1,chance:.3}],xpReward:40},araignee_venimeuse:{id:'araignee_venimeuse',name:'Araignée venimeuse',emoji:'🕷️',baseStats:{for:4,rap:6,con:4,mana:4},drops:[{id:'soie_noire',min:1,max:2,chance:.7},{id:'venin_virulent',min:1,max:1,chance:.5},{id:'patte_chitineuse',min:1,max:2,chance:.45}],xpReward:48},sanglier_cuirasse:{id:'sanglier_cuirasse',name:'Sanglier cuirassé',emoji:'🐗',baseStats:{for:6,rap:3,con:7,mana:1},drops:[{id:'cuir_epais',min:1,max:2,chance:.75},{id:'défense_massive',min:1,max:1,chance:.45},{id:'plaque_osseuse',min:1,max:1,chance:.35}],xpReward:52},serpent_nocturne:{id:'serpent_nocturne',name:'Serpent nocturne',emoji:'🐍',baseStats:{for:5,rap:6,con:4,mana:4},drops:[{id:'écaille_nocturne',min:1,max:2,chance:.7},{id:'croc_venimeux',min:1,max:1,chance:.5},{id:'sang_froid',min:1,max:1,chance:.3}],xpReward:50},treant_ancien:{id:'treant_ancien',name:'Tréant ancien',emoji:'🌳',baseStats:{for:7,rap:2,con:8,mana:4},drops:[{id:'bois_ancien',min:1,max:3,chance:.8},{id:'écorce_runique',min:1,max:1,chance:.45},{id:'sève_vivante',min:1,max:1,chance:.35}],xpReward:62},gobelin_barbare:{id:'gobelin_barbare',name:'Gobelin barbare',emoji:'👹',baseStats:{for:6,rap:5,con:5,mana:1},drops:[{id:'hache_ebrechee',min:1,max:1,chance:.55},{id:'cuir_renforce',min:1,max:2,chance:.65},{id:'os_renforce',min:1,max:2,chance:.5}],xpReward:60},loup_ancien:{id:'loup_ancien',name:'Loup ancien',emoji:'🐺',baseStats:{for:6,rap:7,con:4,mana:2},drops:[{id:'croc_ancien',min:1,max:2,chance:.7},{id:'fourrure_argentee',min:1,max:1,chance:.45},{id:'griffe_aceree',min:1,max:2,chance:.5}],xpReward:64},gardien_ecorce:{id:'gardien_ecorce',name:'Gardien d\'écorce',emoji:'🌲',baseStats:{for:8,rap:3,con:9,mana:5},drops:[{id:'coeur_ecorce',min:1,max:1,chance:.5},{id:'bois_sacre',min:1,max:2,chance:.65},{id:'écorce_runique',min:1,max:2,chance:.55}],xpReward:76},serpent_abyssal:{id:'serpent_abyssal',name:'Serpent abyssal',emoji:'🐍',baseStats:{for:6,rap:7,con:5,mana:6},drops:[{id:'écaille_abyssale',min:1,max:2,chance:.7},{id:'croc_abyssal',min:1,max:1,chance:.5},{id:'venin_noir',min:1,max:1,chance:.45}],xpReward:78},chef_gobelin:{id:'chef_gobelin',name:'Chef gobelin',emoji:'👺',baseStats:{for:7,rap:6,con:6,mana:3},drops:[{id:'insigne_gobelin',min:1,max:1,chance:.45},{id:'lame_chef',min:1,max:1,chance:.35},{id:'cuir_gobelin',min:1,max:3,chance:.75}],xpReward:82},myconide_geant:{id:'myconide_geant',name:'Myconide géant',emoji:'🍄',baseStats:{for:7,rap:3,con:12,mana:8},drops:[{id:'spores_geantes',min:1,max:3,chance:.75},{id:'chapeau_toxique',min:1,max:1,chance:.35},{id:'seve_amere',min:1,max:1,chance:.25}],xpReward:95},feu_follet:{id:'feu_follet',name:'Feu follet',emoji:'🔥',baseStats:{for:4,rap:12,con:5,mana:12},drops:[{id:'flamme_spectrale',min:1,max:2,chance:.75},{id:'essence_lumineuse',min:1,max:1,chance:.4},{id:'eclat_ame',min:1,max:1,chance:.25}],xpReward:98},mandragore:{id:'mandragore',name:'Mandragore',emoji:'🌱',baseStats:{for:6,rap:5,con:10,mana:10},drops:[{id:'racine_hurlante',min:1,max:2,chance:.75},{id:'seve_amere',min:1,max:1,chance:.45},{id:'spores_geantes',min:1,max:2,chance:.35}],xpReward:100},gargouille_moussue:{id:'gargouille_moussue',name:'Gargouille moussue',emoji:'🗿',baseStats:{for:11,rap:3,con:14,mana:5},drops:[{id:'fragment_gargouille',min:1,max:2,chance:.75},{id:'mousse_petrifiee',min:1,max:1,chance:.45},{id:'ronces_solides',min:1,max:1,chance:.3}],xpReward:110},cerf_spectral:{id:'cerf_spectral',name:'Cerf spectral',emoji:'🦌',baseStats:{for:8,rap:12,con:7,mana:10},drops:[{id:'bois_spectral',min:1,max:2,chance:.7},{id:'larme_esprit',min:1,max:1,chance:.35},{id:'essence_lumineuse',min:1,max:1,chance:.3}],xpReward:112},harpie_des_bois:{id:'harpie_des_bois',name:'Harpie des bois',emoji:'🪽',baseStats:{for:10,rap:13,con:6,mana:5},drops:[{id:'plume_tranchante',min:1,max:3,chance:.75},{id:'serre_harpie',min:1,max:1,chance:.4},{id:'bois_spectral',min:1,max:1,chance:.25}],xpReward:115},golem_ronces:{id:'golem_ronces',name:'Golem de ronces',emoji:'🌿',baseStats:{for:13,rap:4,con:16,mana:7},drops:[{id:'ronces_solides',min:1,max:3,chance:.8},{id:'coeur_epines',min:1,max:1,chance:.45},{id:'racine_hurlante',min:1,max:1,chance:.35}],xpReward:125},vouivre_sylvestre:{id:'vouivre_sylvestre',name:'Vouivre sylvestre',emoji:'🐉',baseStats:{for:13,rap:11,con:10,mana:9},drops:[{id:'ecaille_vouivre',min:1,max:2,chance:.75},{id:'croc_vouivre',min:1,max:1,chance:.4},{id:'plume_tranchante',min:1,max:1,chance:.25}],xpReward:130},revenant_sylvestre:{id:'revenant_sylvestre',name:'Revenant sylvestre',emoji:'👻',baseStats:{for:8,rap:10,con:8,mana:14},drops:[{id:'eclat_ame',min:1,max:2,chance:.75},{id:'linceul_ancien',min:1,max:1,chance:.4},{id:'larme_esprit',min:1,max:1,chance:.3}],xpReward:132},salamandre_lave:{id:'salamandre_lave',name:'Salamandre de lave',emoji:'🦎',baseStats:{for:12,rap:13,con:10,mana:10},drops:[{id:'ecaille_lave',min:1,max:2,chance:.75},{id:'coeur_braise',min:1,max:1,chance:.35},{id:'cendre_vive',min:1,max:2,chance:.45}],xpReward:145},elementaire_braise:{id:'elementaire_braise',name:'Élémentaire de braise',emoji:'🔥',baseStats:{for:10,rap:8,con:12,mana:15},drops:[{id:'cendre_vive',min:1,max:3,chance:.75},{id:'coeur_braise',min:1,max:1,chance:.45},{id:'braise_eternelle',min:1,max:1,chance:.18}],xpReward:148},chien_cendres:{id:'chien_cendres',name:'Chien de cendres',emoji:'🐕',baseStats:{for:13,rap:15,con:9,mana:6},drops:[{id:'croc_cendre',min:1,max:2,chance:.75},{id:'cendre_vive',min:1,max:2,chance:.55},{id:'griffe_incandescente',min:1,max:1,chance:.35}],xpReward:150},golem_magma:{id:'golem_magma',name:'Golem de magma',emoji:'🌋',baseStats:{for:16,rap:4,con:18,mana:10},drops:[{id:'fragment_magma',min:1,max:3,chance:.8},{id:'noyau_magmatique',min:1,max:1,chance:.45},{id:'roche_fondue',min:1,max:2,chance:.55}],xpReward:165},serpent_lave:{id:'serpent_lave',name:'Serpent de lave',emoji:'🐍',baseStats:{for:13,rap:16,con:10,mana:12},drops:[{id:'ecaille_obsidienne',min:1,max:2,chance:.75},{id:'venin_bouillant',min:1,max:1,chance:.45},{id:'ecaille_lave',min:1,max:1,chance:.4}],xpReward:168},chauve_souris_incandescente:{id:'chauve_souris_incandescente',name:'Chauve-souris incandescente',emoji:'🦇',baseStats:{for:11,rap:18,con:8,mana:10},drops:[{id:'aile_incandescente',min:1,max:3,chance:.75},{id:'griffe_incandescente',min:1,max:1,chance:.45},{id:'cendre_vive',min:1,max:2,chance:.45}],xpReward:170},vouivre_cendres:{id:'vouivre_cendres',name:'Vouivre de cendres',emoji:'🐉',baseStats:{for:17,rap:14,con:14,mana:13},drops:[{id:'ecaille_cendre',min:1,max:2,chance:.75},{id:'croc_incandescent',min:1,max:1,chance:.45},{id:'braise_eternelle',min:1,max:1,chance:.25}],xpReward:188},colosse_basaltique:{id:'colosse_basaltique',name:'Colosse basaltique',emoji:'🗿',baseStats:{for:19,rap:3,con:22,mana:9},drops:[{id:'fragment_basalte',min:1,max:3,chance:.8},{id:'coeur_basalte',min:1,max:1,chance:.45},{id:'roche_fondue',min:1,max:2,chance:.55}],xpReward:192},spectre_calcine:{id:'spectre_calcine',name:'Spectre calciné',emoji:'👻',baseStats:{for:10,rap:14,con:10,mana:18},drops:[{id:'eclat_calcine',min:1,max:2,chance:.75},{id:'linceul_brule',min:1,max:1,chance:.45},{id:'braise_eternelle',min:1,max:1,chance:.22}],xpReward:195}}
...
const FOREST_ZONES=[
  ...
  {id:'forest_22_24',name:'Bois fongique',label:'Niveaux 22 à 24',minLevel:22,maxLevel:24,monsterPool:['myconide_geant','feu_follet','mandragore']},
  {id:'forest_25_27',name:'Ruines moussues',label:'Niveaux 25 à 27',minLevel:25,maxLevel:27,monsterPool:['gargouille_moussue','cerf_spectral','harpie_des_bois']},
  {id:'forest_28_29',name:'Sanctuaire sauvage',label:'Niveaux 28 à 29',minLevel:28,maxLevel:29,monsterPool:['golem_ronces','vouivre_sylvestre','revenant_sylvestre']},
  {id:'forest_30_32',name:'Bois braisé',label:'Niveaux 30 à 32',minLevel:30,maxLevel:32,monsterPool:['salamandre_lave','elementaire_braise','chien_cendres']},
  {id:'forest_33_35',name:'Gorge de cendres',label:'Niveaux 33 à 35',minLevel:33,maxLevel:35,monsterPool:['golem_magma','serpent_lave','chauve_souris_incandescente']},
  {id:'forest_36_39',name:'Cœur magmatique',label:'Niveaux 36 à 39',minLevel:36,maxLevel:39,monsterPool:['vouivre_cendres','colosse_basaltique','spectre_calcine']}
]
...
else if(s==='buff'){_osc(420,620,'sine',.18,.16);setTimeout(()=>_osc(620,900,'sine',.22,.14),80);setTimeout(()=>_osc(300,500,'triangle',.25,.10),150)}
else if(s==='heal_minor'){_osc(520,760,'sine',.18,.16);setTimeout(()=>_osc(760,980,'sine',.18,.12),90)}
else if(s==='taunt'){_osc(180,120,'square',.18,.18);setTimeout(()=>_osc(240,160,'sawtooth',.20,.14),90);setTimeout(()=>_noise(.10,.08),120)}
else if(s==='bull_stance'){_osc(140,90,'sawtooth',.20,.18);setTimeout(()=>_osc(180,120,'square',.18,.14),90);setTimeout(()=>_osc(260,180,'sawtooth',.22,.12),170)}
else if(s==='clone_dissipate'){_osc(260,120,'triangle',.20,.14);setTimeout(()=>_osc(180,70,'sine',.28,.10),90);setTimeout(()=>_noise(.18,.06),120)}
}
...
function loadGame(slotData){G=JSON.parse(JSON.stringify(slotData));G.combat=G.combat||{monster:null,guarding:false,enemyEffects:{},combatLog:[]};G.tavernResting=false;if(G.selectedForestZoneId===undefined)G.selectedForestZoneId=null;G.team.forEach(c=>{const cls=CLASSES[c.classId];if(cls)c.spells=getUnlockedSpells(cls,c.level);if(c.statPoints===undefined)c.statPoints=0;if(c.statPointsAvailable===undefined)c.statPointsAvailable=(c.statPoints>0)?c.statPoints:0;if(c.statPointsAvailable>0)c.statPoints=0});switchView('map');renderAll();checkPendingStatPoints()}
...
function addCharacter(name,classId,distStats){
  ...
  const char={
    ...
    currentHp:0,currentPe:0,weapon:null,armor:null,spells:getUnlockedSpells(cls,1),
    statPoints:0,statPointsAvailable:0
  }
  const s=getFinalStats(char)
...
function isCombatOver(){
  if(G?.combat?.isBenjyFight)return null
  const m=getMonster()
...
function chooseMonsterTarget(){
  ...
  const dangerous=alive.filter(e=>['mage','sorcier','assassin','epeiste'].includes(e.char.classId))
  const lowestHp=[...alive].sort((a,b)=>getHpRatio(a.char)-getHpRatio(b.char))[0]
  const roll=Math.random()
  if(roll<0.45&&healers.length>0)return healers[Math.floor(Math.random()*healers.length)].char
  if(roll<0.70&&lowestHp)return lowestHp.char
  if(roll<0.90&&dangerous.length>0)return dangerous[Math.floor(Math.random()*dangerous.length)].char
  return alive[Math.floor(Math.random()*alive.length)].char
}
function enemyTurn(){
  if(G?.combat?.isBenjyFight){benjyEnemyTurn();return}
...
function combatSpell(spellId,targetIndex){
  ...
  if(char.cooldowns&&char.cooldowns[spellId]>0){addCombatLog(`⏳ ${spell.name} est encore en recharge (${char.cooldowns[spellId]} tour(s)).`);renderCombat();return}
...
function resolveTurn(){
  ...
  for(const a of actions){
    ...
    if(a.type==='attack'){
      ...
      if(spell.type==='damage'){
        ...
        const actualHeal=target.currentHp-beforeHp
        play('heal_minor')
        addCombatLog(`✨ ${char.name} utilise ${spell.name} sur ${target.name} : +${actualHeal} PV`)
        if(spell.cooldown){char.cooldowns=char.cooldowns||{};char.cooldowns[spell.id]=spell.cooldown}
        renderCombat()
      }else if(spell.type==='heal_formula'){
        const target=G.team[a.targetIndex]
        if(!target||target.currentHp<=0){addCombatLog(`❌ La cible n'est plus valide.`);renderCombat();continue}
        char.currentPe-=spell.cost
        const casterStats=getFinalStats(char)
        const targetStats=getFinalStats(target)
        const heal=Math.floor(spell.baseHeal+(spell.levelScaling*char.level)+(spell.manaScaling*casterStats.mana))
        const beforeHp=target.currentHp
        target.currentHp=Math.min(targetStats.pvMax,target.currentHp+heal)
        const actualHeal=target.currentHp-beforeHp
        play('heal_minor')
        addCombatLog(`✨ ${char.name} utilise ${spell.name} sur ${target.name} : +${actualHeal} PV`)
        if(spell.cooldown){char.cooldowns=char.cooldowns||{};char.cooldowns[spell.id]=spell.cooldown}
        renderCombat()
      }else if(spell.type==='regen_self'){
        char.currentPe-=spell.cost
        char.effects=char.effects||{}
        const healPerTurn=Math.floor(char.level*spell.levelScaling)
        char.effects.regen={value:healPerTurn,duration:spell.duration,source:spell.id}
        play('heal_minor')
        addCombatLog(`🙏 ${char.name} utilise ${spell.name}. Il récupérera ${healPerTurn} PV par tour pendant ${spell.duration} tours.`)
        if(spell.cooldown){char.cooldowns=char.cooldowns||{};char.cooldowns[spell.id]=spell.cooldown}
        renderCombat()
        ...
        play('taunt')
        addCombatLog(`🦁 ${char.name} provoque l'ennemi !`)
        if(spell.cooldown){char.cooldowns=char.cooldowns||{};char.cooldowns[spell.id]=spell.cooldown}
        renderCombat()
  ...
  if(G?.combat?.isBenjyFight&&getMonster()){const m=getMonster(),ms=getMonsterStats();if(m&&ms&&m.currentHp<Math.floor(ms.pvMax*0.90)){m.currentHp=Math.floor(ms.pvMax*0.95);addCombatLog(`Benjy regarde sa blessure. Il soupire.`)}}
  if(getMonster()&&getMonster().currentHp>0){enemyTurn()}
}
function combatFlee(){
  if(!G||!getMonster())return
  if(G?.combat?.isBenjyFight){addCombatLog(`Benjy : "Tu ne pars pas encore."`);renderCombat();return}
...
function victory(){
  ...
  const alive=G.team.filter(c=>c.currentHp>0)
  alive.forEach(c=>{c.xp+=xp;while(c.xp>=xpForLevel(c.level)){c.xp-=xpForLevel(c.level);c.level++;c.statPointsAvailable=(c.statPointsAvailable||0)+1;const ns=getFinalStats(c);c.currentHp=ns.pvMax;c.currentPe=ns.peMax;const cls=CLASSES[c.classId];if(cls)c.spells=getUnlockedSpells(cls,c.level)}})
  const drops=[]
...
function processEnemyEffects(){
  ...
  if(eff.pmRed){eff.pmRed.duration--;if(eff.pmRed.duration<=0)delete eff.pmRed}
}
function getEffectiveMonsterStats(){
  ...
  if(G.combat.enemyEffects?.critRed)result.crit=Math.floor(result.crit*(1-G.combat.enemyEffects.critRed.value/100))
  if(G.combat.enemyEffects?.ppRed)result.pp=Math.floor(result.pp*(1-G.combat.enemyEffects.ppRed.value/100))
  if(G.combat.enemyEffects?.pmRed)result.pm=Math.floor(result.pm*(1-G.combat.enemyEffects.pmRed.value/100))
  return result
}
...
function tickCooldowns(){
  ...
  G.team.forEach(char=>{
    ...
    Object.keys(char.cooldowns).forEach(spellId=>{
      char.cooldowns[spellId]--
      if(char.cooldowns[spellId]<=0)delete char.cooldowns[spellId]
    })
    if(Object.keys(char.cooldowns).length===0)delete char.cooldowns
  })
}
/* ============== FOREST ============== */
let forestZoneTierFilter='1-9'
...
function renderForestZoneMenu(){
  ...
  const visibleZones=FOREST_ZONES.filter(z=>{
    if(forestZoneTierFilter==='1-9')return z.minLevel>=1&&z.maxLevel<=9
    if(forestZoneTierFilter==='10-21')return z.minLevel>=10&&z.maxLevel<=21
    if(forestZoneTierFilter==='22-29')return z.minLevel>=22&&z.maxLevel<=29
    if(forestZoneTierFilter==='30-39')return z.minLevel>=30&&z.maxLevel<=39
    return true
  })
  list.innerHTML=`<div class="forest-zone-filter"><label for="forest-zone-tier-filter">Palier :</label><select id="forest-zone-tier-filter"><option value="1-9"${forestZoneTierFilter==='1-9'?' selected':''}>Niveaux 1 à 9</option><option value="10-21"${forestZoneTierFilter==='10-21'?' selected':''}>Niveaux 10 à 21</option><option value="22-29"${forestZoneTierFilter==='22-29'?' selected':''}>Niveaux 22 à 29</option><option value="30-39"${forestZoneTierFilter==='30-39'?' selected':''}>Niveaux 30 à 39</option></select></div>${visibleZones.map(z=>{const monsterNames=z.monsterPool.map(mid=>{const m=MONSTERS[mid];return m?m.name:mid}).join(', ');return `<div class="forest-zone-card"><div class="forest-zone-text"><div class="forest-zone-title">🌲 ${z.name}</div><div class="forest-zone-label">${z.label}</div><div class="forest-zone-monsters">${monsterNames}</div></div><button class="forest-zone-btn" data-zone="${z.id}">Explorer</button></div>`}).join('')}`
  const sel=document.getElementById('forest-zone-tier-filter')
  if(sel)sel.addEventListener('change',()=>{forestZoneTierFilter=sel.value;renderForestZoneMenu()})
}
...
function merchantSell(){
  ...
  Object.keys(resources).forEach(k=>{
    ...
    html+=`<div class="merchant-item"><div class="mi-info"><div class="mi-name">${r?.emoji||''} ${r?.name||k}</div></div><div class="mi-count">x${resources[k]}</div><div class="mi-price">${price}💰</div><button class="mi-btn" data-sell-res="${k}">Vendre 1</button><button class="mi-btn" data-sell-all-res="${k}">Tout vendre</button></div>`
  })
...
function renderTeamBar(){
  ...
  bar.innerHTML=G.team.map((c,i)=>{
    ...
    const clsObj=CLASSES[c.classId];const pts=c.statPointsAvailable||0;const buffEmojis=getActiveBuffEmojis(c,i)
    return `<div class="team-char ${active} ${dead}" data-char="${i}" style="font-size:0.7rem"><span class="hp-dot" style="background:${c.currentHp<=0?'var(--red)':c.currentHp<s.pvMax*0.3?'var(--orange)':'var(--green)'}"></span>${clsObj?.emoji||''} ${c.name} <span style="color:var(--text2)">Lv.${c.level}</span>${buffEmojis?` ${buffEmojis}`:''} <span style="color:${c.currentHp<=0?'var(--red)':'var(--green)'};font-weight:bold">${c.currentHp}/${s.pvMax}</span>${pts>0?` <span style="color:var(--gold);font-weight:bold;font-size:0.65rem">⬆${pts}</span>`:''}</div>`
  }).join('')
}
...
function renderCombat(){
  ...
  document.getElementById('combat-enemy-name').textContent=`${m.name} (Niv.${m.displayLevel||m.level})`
  ...
  if(char){
    ...
    document.getElementById('combat-ally-name').textContent=`${char.name} Lv.${char.level}${(()=>{const e=getActiveBuffEmojis(char,G.activeChar);return e?' '+e:''})()}`
  ...
  if(char&&!document.getElementById('combat-spells').classList.contains('hidden')){
    spellsList.innerHTML=char.spells.map(sid=>{
      ...
      const canCast=char.currentPe>=sp.cost
        const cooldown=char.cooldowns?.[sid]||0
        return `<button class="spell-btn${(canCast&&!cooldown)?'':' disabled'}" data-spell="${sid}"${(canCast&&!cooldown)?'':' disabled'}>${sp.name} <span class="spell-cost">(${sp.cost} PE${cooldown?` CD:${cooldown}`:''})</span><br><span style="font-size:0.7rem;color:var(--text2)">${getSpellDescription(sp,char)}</span></button>`
    }).join('')
  }
  renderCombatLog()
}
...
function init(){
  ...
  document.querySelectorAll('.map-card:not(.map-locked)').forEach(card=>{
    card.addEventListener('click',()=>{
      ...
      else if(loc==='benjy'){if(G&&G.team.every(c=>c.currentHp<=0)){showMessage('Antre de Benjy','💀 Toute votre équipe est à terre. Reposez-vous à la taverne avant de repartir au combat.');return};showBenjyWarningPopup()}
    })
  })
  /* Explore */
  ...
  document.getElementById('merchant-content').addEventListener('click',(e)=>{
    ...
    }else if(btn.dataset.sellAllRes){
      const id=btn.dataset.sellAllRes;const r=RESOURCES[id];const qty=G.inventory.resources[id]||0
      if(qty>0){const price=r?r.sellPrice:2;G.gold+=price*qty;play('sell');delete G.inventory.resources[id];saveGame();merchantSell();updateGold()}
...
function checkPendingStatPoints(){if(!G||!G.team)return;const idx=G.team.findIndex(c=>(c.statPointsAvailable||0)>0);if(idx>=0)showNextLevelUp()}
function showNextLevelUp(){
  const char=G.team.find(c=>(c.statPointsAvailable||0)>0)
  ...
  const pts=char.statPointsAvailable
...
function showBenjyWarningPopup(){
  const existing=document.getElementById('popup-benjy-warning');if(existing)existing.remove()
  const overlay=document.createElement('div');overlay.id='popup-benjy-warning';overlay.className='popup-overlay'
  overlay.innerHTML='<div class="popup popup-small"><div class="popup-header"><h3>Guilde des Aventuriers</h3><button class="popup-close" id="btn-benjy-warning-close">✕</button></div><div class="popup-body" style="text-align:center;font-size:0.9rem;line-height:1.6"><p>La Guilde déconseille formellement l\'accès à ce lieu.</p><p>Cette zone n\'est pas cartographiée.</p><p>Aucune récompense n\'est garantie.</p><p>Aucun retour n\'est assuré.</p><p style="color:var(--gold);font-weight:bold;margin-top:12px">Entrer quand même ?</p></div><div class="popup-footer"><button id="btn-benjy-enter" class="btn-primary">Entrer</button><button id="btn-benjy-cancel" class="btn-secondary">Annuler</button></div></div>'
  document.body.appendChild(overlay)
  document.getElementById('btn-benjy-enter').addEventListener('click',()=>{overlay.remove();startBenjyCombat()})
  document.getElementById('btn-benjy-cancel').addEventListener('click',()=>{overlay.remove()})
  document.getElementById('btn-benjy-warning-close').addEventListener('click',()=>{overlay.remove()})
  overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove()})
}
...
function startBenjyCombat(){
  ...
  addCombatLog(`👉 ${G.team[G.activeChar].name}, à toi !`)
  switchView('combat');renderCombat()
}
function formatNameList(names){if(names.length===0)return '';if(names.length===1)return names[0];if(names.length===2)return `${names[0]} et ${names[1]}`;return `${names.slice(0,-1).join(', ')} et ${names[names.length-1]}`}
function getAliveTeamEntries(){return G.team.map((char,index)=>({char,index})).filter(e=>e.char.currentHp>0)}
function damagePercentOfMax(char,percent,minimumHp=0){const s=getFinalStats(char);const dmg=Math.max(1,Math.floor(s.pvMax*percent));char.currentHp=Math.max(minimumHp,char.currentHp-dmg);return dmg}
...
function chooseBenjyOpeningTarget(){
  ...
  const healers=alive.filter(e=>e.char.classId==='guerisseur')
  if(healers.length>0)return{target:healers[Math.floor(Math.random()*healers.length)],kill:true,reason:'healer'}
  if(alive.length<=2)return{target:alive[Math.floor(Math.random()*alive.length)],kill:false,reason:'small_team'}
  const nonTanks=alive.filter(e=>e.char.classId!=='tank')
  if(nonTanks.length>0)return{target:nonTanks[Math.floor(Math.random()*nonTanks.length)],kill:true,reason:'non_tank'}
  return{target:alive[Math.floor(Math.random()*alive.length)],kill:true,reason:'all_tanks'}
}
...
function benjyUsesTankAsWeapon(){
  ...
  if(!tankEntry||alive.length<2){benjyWallThrow();return}
  const possibleTargets=alive.filter(e=>e.index!==tankEntry.index)
...
function checkBenjyEndCondition(){
  ...
  const allUnderTenPercent=isTeamUnderBenjyLimit()
  if(allDead||allUnderTenPercent){endBenjyFight();return true}
  return false
}
function startNextTurnAfterBenjy(){
  if(!G||!G.combat?.isBenjyFight)return
  tickCharacterEffects();tickCooldowns();G.combat.charActed=G.team.map(()=>false)
  const aliveIdx=G.team.findIndex(c=>c.currentHp>0)
...
function benjyEnemyTurn(){
  ...
  if(!G.combat.benjyOpeningDone){benjyOpeningAction();checkBenjyEndCondition();if(!G.combat.monster)return;startNextTurnAfterBenjy();return}
  if(G.combat.benjyTurnCount===2)addCombatLog(`Benjy : "C'est tout ?"`)
...
function endBenjyFight(){
  if(!G||!G.combat?.isBenjyFight)return
  const deadBefore=G.team.filter(c=>c.currentHp<=0);const deadNames=deadBefore.map(c=>c.name);const namesText=formatNameList(deadNames)
  addCombatLog(`Benjy : "Bon. Assez."`);addCombatLog(`Benjy claque des doigts.`)
  if(deadBefore.length===1){addCombatLog(`${namesText} reprend conscience avec 1% de PV.`);addCombatLog(`Benjy saisit le corps par le col et le jette hors de son antre avec les autres.`)}
  else if(deadBefore.length>1){addCombatLog(`${namesText} reprennent conscience avec 1% de PV.`);addCombatLog(`Benjy les traîne par le col et les jette hors de son antre avec les autres.`)}
  else addCombatLog(`Une force écrasante repousse votre équipe hors de son antre.`)
  G.team.forEach(c=>{if(c.currentHp<=0){const s=getFinalStats(c);c.currentHp=Math.max(1,Math.floor(s.pvMax*0.01))}})
  addCombatLog(`Votre équipe est expulsée de l'Antre de Benjy.`)
  saveGame();renderCombat()
  setTimeout(()=>{
    if(!G)return
    G.combat={monster:null,guarding:false,enemyEffects:{},combatLog:[],charActed:[],pendingActions:[]}
    switchView('map');renderAll()
    showMessage('Antre de Benjy','Benjy : "Bon. Assez. Dégagez de chez moi."<br><br>☠️ Benjy vous a expulsés de son antre.')
  },2500)
}
